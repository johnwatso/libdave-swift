...
existing client needs a code or deployment migration. The current hardening
batch should ship as `2.0.0`: it intentionally changes source and runtime
integration contracts, including public enum cases, diagnostics data, media
readiness, and gateway action delivery.
...
```sh
git tag -a 2.0.0 -m "libdave-swift 2.0.0"
